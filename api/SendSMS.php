...
class SMS {
    ...
    function SMS($da = "", $sa = "", $msg = "", $sat = "", $dcs = "", $dr = "", $udh = "", $ur = "", $vp = "", $du = "", $lt = "", $un = "", $p = "") {
        global $errstr;
        $errstr = "Construction failed.";

        if (!$this->setUN($un)) //return false;
        $errstr = "DA, no error";
        if (!$this->setP($p)) //return false;
        $errstr = "DA, no error";
...
function send_sms_object ($sms) {
    global $systemtype, $errstr, $errcode;
    ...
    $request = "http://sms1.cardboardfish.com:9001/HTTPSMS?S={$systemtype}&UN={$sms->username}&P={$sms->password}&DA={$sms->dest_addr}&SA={$sms->source_addr}&M=${msg}";
